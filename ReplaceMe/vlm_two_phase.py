# ============================================================
# vlm_two_phase.py - Two-Phase Teacher-Student Optimization
...
def collect_layer_outputs(
    ...
    Args:
        model: Full (unpruned) model
        dataloader: Calibration dataloader
        layer_before: Layer index before pruning (e.g., 19)
        layer_after: Layer index after skip (e.g., 23)
        max_samples: Maximum samples to collect (for memory)
        
    Returns:
        L_before: Layer outputs before [N, d]
        L_after: Layer outputs after [N, d]
    """
    print(f"\n{Fore.MAGENTA}{'='*70}{Fore.RESET}")
    ...
    # Pre-allocate
    if max_samples:
        estimated_tokens = max_samples * 1000
    else:
        estimated_tokens = len(dataloader.dataset) * 1000
    ...
    for batch_idx, batch in enumerate(tqdm(
        ...
        if max_samples and batch_idx * batch['input_ids'].shape[0] >= max_samples:
        ...
        # So hidden_states[layer_idx + 1] = layer output
        L_before_batch = outputs.hidden_states[layer_before + 1]  # +1 for embedding offset
        ...
        if batch_idx == 0:
            print(f"\n{Fore.YELLOW}[DEBUG Teacher] First batch:{Fore.RESET}")
            print(f"  Input shape: {inputs['input_ids'].shape}")
            print(f"  L_before (layer {layer_before}): {L_before_batch.shape}")
            print(f"  L_after (layer {layer_after}): {L_after_batch.shape}\n")
        
        L_before_list.append(L_before_batch)
        L_after_list.append(L_after_batch)
        
        total_tokens += L_before_batch.shape[0]
...
def optimize_phase2_teacher_guided(
    ...
    Layer-level matching: L_before @ T ≈ L_after
    """
    print(f"\n{Fore.MAGENTA}{'='*70}{Fore.RESET}")
    ...
    print(f"\n{Fore.CYAN}Key Difference from Phase 1:{Fore.RESET}")
    print(f"  Phase 1: MLP output → Target layer (intermediate)")
    print(f"  Phase 2: Full layer output → Target layer (end-to-end)")
    print(f"  Initialize with T_base for stability\n")
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx in pbar:
            ...
            if batch_idx == 0 and epoch == 0:
                print(f"\n{Fore.YELLOW}[DEBUG Phase 2] First batch:{Fore.RESET}")
                print(f"  L_before: {L_before_batch.shape}, {L_before_batch.dtype}")
                print(f"  L_after: {L_after_batch.shape}, {L_after_batch.dtype}")
                print(f"  T: {transform.shape}, {transform.dtype}\n")
            
            # Forward: transform layer output
...
def ensemble_transforms(
    ...
    print(f"  T_base: MLP-level matching (Phase 1)")
    print(f"  T_refined: Layer-level matching with teacher (Phase 2)")
    print(f"  Weighted combination for robustness\n")
...
def vlm_two_phase(
    ...
    """
    VLM pruning with Two-Phase Teacher-Student optimization.
    
    Phase 1: Standard ReplaceMe (MLP-level)
    Phase 2: Teacher-guided refinement (Layer-level)
    """
    print(f"\n{Fore.MAGENTA}{'='*70}{Fore.RESET}")
    print(f"{Fore.MAGENTA}VLM TWO-PHASE: TEACHER-STUDENT OPTIMIZATION{Fore.RESET}")
    print(f"{Fore.MAGENTA}{'='*70}{Fore.RESET}\n")
    ...
    print(f"  Skip distance: {target_layer_after - target_layer_before} layers\n")
    
    # ===== LOAD MODEL FOR PHASE 1 =====
    print(f"{Fore.GREEN}[Step 1/9] Loading model for Phase 1...{Fore.RESET}")
    ...
    # Note: model is still loaded (full unpruned model)
    ...
    apply_vlm_transform(model, T_final, start_id - num_layer - 1)
