...
from .multiscale_cosine_dist import multiscale_cosine_dist  # New import
...
def ReplaceMe_pipeline(config):
    ...
    elif config["method"] == "multiscale_cosine":
        # New Multi-Scale Calibration Data Enhancement method
        logging.info(f"{Fore.GREEN}Using Multi-Scale Calibration Data Enhancement{Fore.RESET}")
        signature = inspect.signature(multiscale_cosine_dist)
        filtered_config = {k: v for k, v in config.items() if k in signature.parameters}
