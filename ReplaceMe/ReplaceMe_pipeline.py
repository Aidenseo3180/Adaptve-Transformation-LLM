...
def ReplaceMe_pipeline(config):
    ...
    elif config["method"] == "vib":
        # VIB 방법: cosine_dist 함수를 재사용하되 VIB loss 사용
        signature = inspect.signature(cosine_dist)
        filtered_config = {k: v for k, v in config.items() if k in signature.parameters}
        
        # VIB 전용 파라미터 추가
        if 'beta' not in filtered_config:
            filtered_config['beta'] = config.get('beta', 0.1)  # 기본값 0.1
        
        # loss를 "vib"로 강제 설정
        filtered_config['loss'] = "vib"
        filtered_config['solver'] = config.get('solver', 'adam')  # VIB는 Adam 사용 권장
