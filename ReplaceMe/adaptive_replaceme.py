...
def fixed_adaptive_replaceme(
    ...
    num_A: int = 1,
    merge_consecutive: bool = True,
    max_tokens_to_collect: int = 500000,  # Maximum tokens to collect
    **kwargs
) -> str:
    
    print(f"\n[Fixed AR] Processing layers {start_id}-{end_id}")
    print(f"[Fixed AR] Skip size: {end_id - start_id} layers")
    print(f"[Fixed AR] Max tokens to collect: {max_tokens_to_collect}")
    ...
    for batch in tqdm(dataloader, desc="Calibration"):
        ...
        # Compute what we need to match (EXACTLY LIKE ORIGINAL)
        # a1 = MLP output, a2 = target (h_after + mlp_out - h_before)
        a1 = mlp_out.view(-1, hidden_size)
        a2 = (h_after + mlp_out - h_before).view(-1, hidden_size)
        ...
        if total_tokens_collected + a1.shape[0] > max_tokens_to_collect:
            # Only take as many tokens as needed to reach limit
            ...
            print(f"[Fixed AR] Reached token limit: {total_tokens_collected} tokens")
            break
        else:
            a1_list.append(a1.cpu())
            a2_list.append(a2.cpu())
    ...
    chunk_size = min(50000, a1.shape[0])  # Process max 50k tokens at a time
    
    best_W = W.clone()
    best_loss = float('inf')
    
    for epoch in range(20): #10
        total_loss = 0
        num_batches = 0
        ...
        for i in range(0, len(indices), chunk_size):
            ...
            # Further split into mini-batches for gradient computation
