import argparse
import gc
import logging
import os
from typing import Optional
import torch
import yaml
from colorama import Fore, init
from tqdm import tqdm
from transformers import (AutoModelForCausalLM, AutoTokenizer,
                          BitsAndBytesConfig)

from .utils import (adam_method, get_calib_dataloader, optimizing_method,
                    select_non_overlapping_blocks, truncate_model, seed_all)

# Initialize colorama for Windows compatibility
init(autoreset=True)

# Configure logging to display colored messages and timestamps
logging.basicConfig(
    format=f'{Fore.CYAN}%(asctime)s {Fore.YELLOW}[%(levelname)s] {Fore.RESET}%(message)s',
    level=logging.INFO,
    datefmt='%Y-%m-%d %H:%M:%S'
)

seed_all()


def compute_dimension_importance(model, dataloader, tokenizer, start_id, end_id, num_layer, max_samples=1000):
    """
    Compute importance weights for each hidden dimension based on prediction impact.
    
    Args:
        model: The transformer model
        dataloader: Calibration data loader
        tokenizer: Model tokenizer
        start_id: Starting layer ID
        end_id: Ending layer ID
        num_layer: Number of layers processed so far
        max_samples: Maximum number of samples for importance computation
        
    Returns:
        importance_weights: Tensor of shape (hidden_dim,) with importance scores
    """
    print(f"[DEBUG] Computing dimension importance for layers {start_id} to {end_id}")
    
    model.eval()
    hidden_dim = model.config.hidden_size
    importance_weights = torch.zeros(hidden_dim, dtype=torch.float32)
    total_samples = 0
    
    def save_activations(name):
        def hook(module, input, output):
            activations[name] = output.detach()
        return hook
    
    # Register hooks for the layers we care about
    activations = {}
    hooks = []
    
    if 'falcon' in str(model.config._name_or_path).lower():
        target_layer = model.transformer.h[start_id - num_layer - 1]
    else:
        target_layer = model.model.layers[start_id - num_layer - 1]
    
    # Hook on the MLP output of the target layer
    hooks.append(target_layer.mlp.register_forward_hook(save_activations('target_mlp')))
    
    print(f"[DEBUG] Registered hooks for importance computation")
    
    sample_count = 0
    for batch in tqdm(dataloader, desc="Computing Dimension Importance", dynamic_ncols=True):
        if sample_count >= max_samples:
            break
            
        inputs = tokenizer(
            batch,
            return_tensors="pt",
            padding="longest",
            max_length=512,  # Shorter for importance computation
            truncation=True
        )
        inputs = {k: v.to(model.device) for k, v in inputs.items()}
        
        with torch.no_grad():
            # Forward pass
            outputs = model(**inputs)
            
            # Get the MLP activations
            mlp_output = activations['target_mlp']  # Shape: (batch, seq, hidden_dim)
            
            # Compute per-dimension variance as importance proxy
            # Higher variance = more important for different inputs
            dim_variance = torch.var(mlp_output.view(-1, hidden_dim), dim=0)
            importance_weights += dim_variance.cpu()
            
            total_samples += mlp_output.shape[0] * mlp_output.shape[1]  # batch * seq_len
            sample_count += len(batch)
    
    # Clean up hooks
    for hook in hooks:
        hook.remove()
    
    # Normalize importance weights
    importance_weights = importance_weights / sample_count
    importance_weights = importance_weights / torch.sum(importance_weights) * hidden_dim  # Normalize to mean=1
    
    print(f"[DEBUG] Dimension importance computed from {total_samples} tokens")
    print(f"[DEBUG] Importance stats - min: {importance_weights.min():.6f}, max: {importance_weights.max():.6f}, mean: {importance_weights.mean():.6f}")
    print(f"[DEBUG] Top 5 most important dimensions: {torch.topk(importance_weights, 5).indices.tolist()}")
    print(f"[DEBUG] Top 5 importance values: {torch.topk(importance_weights, 5).values.tolist()}")
    
    return importance_weights


def token_weighted_svd_factorization(T: torch.Tensor, importance_weights: torch.Tensor, rank: int = 1024) -> tuple:
    """
    Perform token-weighted SVD factorization on transformation matrix T.
    
    Args:
        T: Original transformation matrix (d x d)
        importance_weights: Importance weights for each dimension (d,)
        rank: Target rank for decomposition
        
    Returns:
        U: Left factor matrix (d x rank)
        V: Right factor matrix (d x rank)
    """
    print(f"[DEBUG] Starting token-weighted SVD factorization")
    print(f"[DEBUG] Original T shape: {T.shape}")
    print(f"[DEBUG] Importance weights shape: {importance_weights.shape}")
    print(f"[DEBUG] Target rank: {rank}")
    
    # Move to CPU for computation
    T_cpu = T.cpu().float()
    weights_cpu = importance_weights.cpu().float()
    
    # Create weighting matrix - we weight both rows and columns
    # Higher importance = higher weight in reconstruction
    W = torch.sqrt(weights_cpu).unsqueeze(0)  # Shape: (1, d)
    W_row = W.T  # Shape: (d, 1)
    W_col = W    # Shape: (1, d)
    
    print(f"[DEBUG] Weight matrix stats - min: {weights_cpu.min():.6f}, max: {weights_cpu.max():.6f}")
    
    # Apply weighting: T_weighted = W_row * T * W_col
    T_weighted = (W_row * T_cpu) * W_col
    
    print(f"[DEBUG] Weighted T norm: {torch.norm(T_weighted):.6f} vs Original T norm: {torch.norm(T_cpu):.6f}")
    
    # Perform SVD on weighted matrix
    U_w, S, Vt_w = torch.svd(T_weighted)
    print(f"[DEBUG] SVD completed on weighted matrix")
    print(f"[DEBUG] Top 10 singular values: {S[:10].tolist()}")
    
    # Take top-k components
    U_lr = U_w[:, :rank].contiguous()
    S_lr = S[:rank]
    V_lr = Vt_w[:rank, :].T.contiguous()
    
    # Scale by singular values
    U_lr = U_lr @ torch.diag(torch.sqrt(S_lr))
    V_lr = V_lr @ torch.diag(torch.sqrt(S_lr))
    
    # Adjust back for weighting - remove the weighting effect
    U_lr = U_lr / W_row  # Remove row weighting
    V_lr = V_lr / W_col.T  # Remove column weighting
    
    print(f"[DEBUG] After rank reduction and weight adjustment - U_lr: {U_lr.shape}, V_lr: {V_lr.shape}")
    
    # Verify reconstruction
    T_reconstructed = U_lr @ V_lr.T
    
    # Compute both weighted and unweighted reconstruction errors
    weighted_error = torch.norm((W_row * (T_cpu - T_reconstructed)) * W_col) / torch.norm((W_row * T_cpu) * W_col)
    unweighted_error = torch.norm(T_cpu - T_reconstructed) / torch.norm(T_cpu)
    
    print(f"[DEBUG] Reconstruction verification:")
    print(f"[DEBUG] - Weighted reconstruction error: {weighted_error:.6f}")
    print(f"[DEBUG] - Unweighted reconstruction error: {unweighted_error:.6f}")
    print(f"[DEBUG] - Original T norm: {torch.norm(T_cpu):.6f}")
    print(f"[DEBUG] - Reconstructed T norm: {torch.norm(T_reconstructed):.6f}")
    
    # Return to original dtype
    result_U = U_lr.to(T.dtype)
    result_V = V_lr.to(T.dtype)
    
    print(f"[DEBUG] Token-weighted SVD factorization completed successfully")
    
    return result_U, result_V


def apply_token_weighted_transform(model, layer_idx: int, U: torch.Tensor, V: torch.Tensor):
    """
    Apply token-weighted low-rank transformation to model by modifying down_proj weights.
    
    Args:
        model: The transformer model
        layer_idx: Index of the layer to modify
        U: Left factor matrix
        V: Right factor matrix
    """
    print(f"[DEBUG] Applying token-weighted transform to layer {layer_idx}")
    
    # Get the down_proj layer
    down_proj = model.model.layers[layer_idx].mlp.down_proj
    original_weight = down_proj.weight.data
    
    print(f"[DEBUG] Original down_proj weight shape: {original_weight.shape}")
    print(f"[DEBUG] Original down_proj weight device: {original_weight.device}")
    print(f"[DEBUG] Input U shape: {U.shape}, V shape: {V.shape}")
    
    # Store original weight norm for comparison
    original_norm = torch.norm(original_weight).item()
    print(f"[DEBUG] Original weight norm: {original_norm:.6f}")
    
    # Ensure all tensors are on the same device
    device = original_weight.device
    dtype = original_weight.dtype
    
    U = U.to(device=device, dtype=torch.float64)
    V = V.to(device=device, dtype=torch.float64)
    original_weight_fp64 = original_weight.to(torch.float64)
    
    print(f"[DEBUG] Computing token-weighted transformation: V @ U.T @ original_weight")
    
    # Apply transformation: W_new = V @ U^T @ W_original
    intermediate = U.T @ original_weight_fp64
    transformed_weight = V @ intermediate
    
    # Verify transformation
    transformed_norm = torch.norm(transformed_weight).item()
    print(f"[DEBUG] Transformed weight norm: {transformed_norm:.6f}")
    print(f"[DEBUG] Weight norm ratio (new/old): {transformed_norm/original_norm:.6f}")
    
    # Update the weight
    down_proj.weight.data = transformed_weight.to(dtype)
    
    # Verify the update
    new_weight = down_proj.weight.data
    new_norm = torch.norm(new_weight).item()
    print(f"[DEBUG] After assignment - new weight norm: {new_norm:.6f}")
    print(f"[DEBUG] Assignment verification: {torch.allclose(new_weight.float(), transformed_weight.float(), atol=1e-5)}")
    
    print(f"[DEBUG] Successfully applied token-weighted transform to layer {layer_idx}")
    logging.info(f"{Fore.GREEN}Applied token-weighted transform to layer {layer_idx}{Fore.RESET}")


def token_weighted_replace(
    model_path: str,
    dataset: str,
    dataset_column: str,
    batch_size: int,
    max_length: int,
    layers_to_skip: int,
    dataset_size: Optional[int] = None,
    dataset_subset: Optional[str] = "eval",
    activations_save_path: Optional[str] = None,
    use_4bit: bool = False,
    save_path: Optional[str] = None,
    min_distance_layer: Optional[int] = None,
    token: Optional[str] = None,
    save_transform_only: bool = False,
    diag: bool = False,
    loss: str = "cosine",
    solver: str = "adam",
    thri: bool = False,
    two_vectors: bool = False,
    start_id: int = 0,
    end_id: int = 0,
    num_layer: int = 0,
    distances_path: str = "./distances.pth",
    num_A: int = 1,
    merge_consecutive: bool = True,
    accurate: bool = False,
    low_rank: int = 1024
) -> str:
    """Calculate cosine distance between model layers and apply token-weighted transformations.
    
    Args:
        Same as low_rank_replace with additional token-weighting
    
    Returns:
        Path where transformed model is saved
    """
    device_map = "auto" if torch.cuda.is_available() else "cpu"
    quantization_config = None
    
    if use_4bit:
        quantization_config = BitsAndBytesConfig(
            load_in_4bit=True,
            bnb_4bit_use_double_quant=True,
            bnb_4bit_quant_type="nf4",
            bnb_4bit_compute_dtype=torch.bfloat16
        )

    model = AutoModelForCausalLM.from_pretrained(
        model_path,
        device_map=device_map,
        quantization_config=quantization_config,
        output_hidden_states=True,
        token=token
    )

    tokenizer = AutoTokenizer.from_pretrained(model_path)
    hidden_size = model.config.hidden_size
    
    if not tokenizer.pad_token:
        tokenizer.pad_token = tokenizer.eos_token
    
    model.eval()
    dataloader = get_calib_dataloader(
        dataset,
        dataset_subset,
        dataset_column,
        dataset_size,
        batch_size,
        tokenizer
    )
    
    # Step 1: Compute dimension importance weights
    print(f"[DEBUG] Step 1: Computing dimension importance weights")
    importance_weights = compute_dimension_importance(
        model, dataloader, tokenizer, start_id, end_id, num_layer, max_samples=500
    )
    
    def save_mlp_activation(name):
        """Returns a hook function that saves the module output under the key 'name'."""
        def hook(module, input, output):
            mlp_activations[name] = output.detach()
        return hook

    hooks = []
    if 'falcon' in model_path.lower():
        for i, layer in enumerate(model.transformer.h):
            hooks.append(layer.mlp.register_forward_hook(save_mlp_activation(f'layer_{i}_mlp')))
    else:
        for i, layer in enumerate(model.model.layers):
            hooks.append(layer.mlp.register_forward_hook(save_mlp_activation(f'layer_{i}_mlp')))

    mlp_activations = {}
    a1 = torch.empty(
        (dataset_size * max_length, model.config.hidden_size),
        dtype=torch.bfloat16,
        device='cpu'
    )
    a2 = torch.empty(
        (dataset_size * max_length, model.config.hidden_size),
        dtype=torch.bfloat16,
        device='cpu'
    )
    if accurate:
        print("ACCURATE MODE IS ON (MORE MEMORY IS NEEDED)")
        a3 = torch.empty(
            (dataset_size * max_length, model.config.hidden_size),
            dtype=torch.bfloat16,
            device='cpu'
        )
    
    cnt = 0
    print(f"[DEBUG] Step 2: Gathering activations for transformation matrix estimation")
    for batch in tqdm(
        dataloader,
        desc=Fore.RED + "Gathering Activations" + Fore.RESET,
        dynamic_ncols=True,
        colour="red"
    ):
        inputs = tokenizer(
            batch,
            return_tensors="pt",
            padding="longest",
            max_length=max_length,
            truncation=True
        )
        inputs = {k: v.to(model.device) for k, v in inputs.items()}
        
        with torch.no_grad():
            outputs = model(**inputs)
        
        hidden_states = outputs.hidden_states[1:]
        hidden_states_mlp_list = [
            mlp_activations[f'layer_{i}_mlp'] for i in range(model.config.num_hidden_layers)
        ]
        hidden_states_mlp = hidden_states_mlp_list[start_id - num_layer - 1]

        # Reshape activations
        hidden_states_mlp = hidden_states_mlp.view(-1, hidden_size).to(torch.float64)
        hidden_states_i = hidden_states[start_id - num_layer - 1].view(-1, hidden_size).to(torch.float64)
        hidden_states_n = hidden_states[end_id - num_layer - 1].view(-1, hidden_size).to(torch.float64)
        
        a1_batch = hidden_states_mlp
        a2_batch = hidden_states_n + hidden_states_mlp - hidden_states_i
        if accurate:
            a2_batch = hidden_states_n 
            a3_batch = hidden_states_i - hidden_states_mlp 
            a3[cnt:cnt+a3_batch.shape[0]] = a3_batch
        a1[cnt:cnt+a1_batch.shape[0]] = a1_batch
        a2[cnt:cnt+a2_batch.shape[0]] = a2_batch
        
        cnt += a2_batch.shape[0]
        
        del hidden_states_mlp, hidden_states_i, hidden_states_n
    
    a1 = a1[:cnt]
    a2 = a2[:cnt]
    if accurate:
        a3 = a3[:cnt]
    
    # Step 3: Compute full-rank transformation matrix
    print(f"[DEBUG] Step 3: Computing full-rank transformation matrix T")
    print(f"[DEBUG] Calibration data processed: {cnt} tokens")
    
    if solver == "adam":
        print(f"[DEBUG] Using Adam solver with loss: {loss}")
        transform = adam_method(a1, a2, a3=a3 if accurate else None, loss=loss, diag=diag, two_vectors=two_vectors, thri=thri)
    else:
        print(f"[DEBUG] Using optimization method with solver: {solver}")
        transform = optimizing_method(a1, a2, a3=a3 if accurate else None, solver=solver)
    
    print(f"[DEBUG] Full-rank transformation T computed")
    print(f"[DEBUG] T shape: {transform.shape}, T norm: {torch.norm(transform):.6f}")
    
    # Step 4: Apply token-weighted SVD factorization
    print(f"[DEBUG] Step 4: Applying token-weighted SVD factorization")
    U, V = token_weighted_svd_factorization(transform, importance_weights, rank=low_rank)
    
    # Clean up and reload model
    print(f"[DEBUG] Step 5: Reloading model and applying transformation")
    del model
    gc.collect()
    torch.cuda.empty_cache()
    
    model = AutoModelForCausalLM.from_pretrained(
        model_path,
        device_map='cpu',
        torch_dtype=torch.bfloat16
    )
    model = truncate_model(model, start_id - num_layer, end_id - num_layer)
    
    if save_path is None:
        if not os.path.exists('output_models'):
            os.mkdir('output_models')
        save_path = "output_models/" + (
            f"{model_path}_{layers_to_skip}_layers_{start_id}_"
            f"{end_id}_{dataset}_{dataset_size}_tokenweighted_{low_rank}"
        ).replace("/", "_")
    
    # Apply token-weighted transformation
    target_layer = start_id - num_layer - 1
    print(f"[DEBUG] Applying token-weighted transformation to layer index: {target_layer}")
    apply_token_weighted_transform(model, target_layer, U, V)
    
    final_path = f"{save_path}_TokenWeighted_{loss}_{solver}_r{low_rank}"
    model.save_pretrained(final_path)
    tokenizer.save_pretrained(final_path)
    print(f"[DEBUG] Model saved to: {final_path}")
    
    if save_transform_only:
        torch.save({
            'U': U,
            'V': V,
            'importance_weights': importance_weights,
            'rank': low_rank,
            'original_transform': transform
        }, f"{final_path}_factors")
        print(f"[DEBUG] Transform factors saved")
    
    # Final cleanup
    print(f"[DEBUG] Final cleanup")
    del model, a1, a2
    if accurate:
        del a3
    gc.collect()
    torch.cuda.empty_cache()
    
    print(f"[DEBUG] Token-weighted replacement completed successfully")
    print(f"[DEBUG] Final model path: {final_path}")
    
    return final_path

