...
def cosine_distance(x_l: torch.Tensor, x_l_plus_n: torch.Tensor) -> torch.Tensor:
    x_l_norm = x_l / torch.norm(x_l, dim=-1, keepdim=True)
    x_l_plus_n_norm = x_l_plus_n / torch.norm(x_l_plus_n, dim=-1, keepdim=True)
    cosine_sim = (x_l_norm * x_l_plus_n_norm).sum(-1)
    return 1 - cosine_sim  # 이게 ReplaceMe 방식
...
def vlm_profile_distances(
    ...
    max_length: int = 512,
    layers_to_skip: int = 8,
    dataset_size: Optional[int] = 8000,
    use_4bit: bool = False,
    token: Optional[str] = None,
) -> List[float]:
    """
