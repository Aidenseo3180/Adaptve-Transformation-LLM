...
def test_llava_15_13b():
  ...
  CONFIG = {
      ...
      # Analysis
      'standard_layers': [5, 7, 10, 13, 15, 17, 20, 22, 25, 27, 30, 32, 35, 38, 39],
      'fine_grained_layers': list(range(13, 22)),  # Layer 17 deep dive
  ...
  # Test 1: Layer 17 vs neighbors (is it significantly different?)
  print("\n1. Layer 17 vs Neighbors (Cosine Similarity)")
  print("   Testing if Layer 17 dip is statistically significant")
  print("   " + "─" * 76)

  layer_17_values = aggregated_results[17]['cosine_similarity']

  for neighbor in [15, 16, 18, 19]:
      if neighbor in aggregated_results:
          neighbor_values = aggregated_results[neighbor]['cosine_similarity']
          
          # Two-sample t-test
          t_stat, p_value = stats.ttest_ind(layer_17_values, neighbor_values)
          
          # Effect size (Cohen's d)
          mean_diff = np.mean(layer_17_values) - np.mean(neighbor_values)
          pooled_std = np.sqrt(
              (np.std(layer_17_values)**2 + np.std(neighbor_values)**2) / 2
          )
          cohens_d = mean_diff / pooled_std if pooled_std > 0 else 0
          
          statistical_tests[f'layer17_vs_{neighbor}'] = {
              't_statistic': float(t_stat),
              'p_value': float(p_value),
              'cohens_d': float(cohens_d),
              'mean_diff': float(mean_diff),
